...
import API_ENDPOINTS, { ensureAuthToken, getAuthHeaders } from "@/lib/api";
...
export default function Adapters() {
  ...
  const fetchSources = async () => {
    try {
      ...
      const res = await axios.get(API_ENDPOINTS.ADAPTERS_LIST, config);
  ...
  const handleActivateSource = async () => {
    ...
    const sourceName =
      formData.connectionType === "file" && uploadedFile
        ? uploadedFile.name
    ...
    try {
      ...
      const res = await axios.post(API_ENDPOINTS.ADAPTERS_CREATE, payload, {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage active protocol translators mapping legacy formats to
            standard JSON.
          </p>
        </div>
      ...
      {showAddForm && (
        <Card className="border-primary/50 bg-primary/5">
          <CardHeader>
            ...
            <CardDescription>
              Establish a connection to a new legacy system or upload a file
              containing legacy formats.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <label
                  className={`flex flex-1 items-start gap-3 rounded-lg border p-4 cursor-pointer transition-all duration-200 ${formData.mappingMode === "ai" ? "border-primary bg-primary/5 shadow-sm shadow-primary/5" : "border-card-border bg-card hover:border-white/20"}`}
                >
                  <input
                    type="radio"
                    name="mappingMode"
                    value="ai"
                    checked={formData.mappingMode === "ai"}
                    onChange={() =>
                      setFormData({ ...formData, mappingMode: "ai" })
                    }
                    className="mt-1 accent-primary"
                  />
                  <div>
                    <div className="font-medium text-white">
                      AI Handles It (Recommended)
                    </div>
                    <div className="text-sm text-muted-foreground mt-1">
                      Automatically infers schema and semantic mappings in
                      real-time.
                    </div>
                  </div>
                </label>
                <label
                  className={`flex flex-1 items-start gap-3 rounded-lg border p-4 cursor-pointer transition-all duration-200 ${formData.mappingMode === "manual" ? "border-primary bg-primary/5 shadow-sm shadow-primary/5" : "border-card-border bg-card hover:border-white/20"}`}
                >
                  <input
                    type="radio"
                    name="mappingMode"
                    value="manual"
                    checked={formData.mappingMode === "manual"}
                    onChange={() =>
                      setFormData({ ...formData, mappingMode: "manual" })
                    }
                    className="mt-1 accent-primary"
                  />
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground mt-1">
                      Define strict field-to-field transformation rules.
                    </div>
                  </div>
                </label>
              </div>
            ...
            {formData.mappingMode === "manual" && (
              <div className="space-y-2 animate-in slide-in-from-top-2">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, manualMapping: e.target.value })
                  }
                />
              </div>
            )}
            ...
            <div className="flex items-center gap-4 pt-4 border-t border-card-border">
              ...
              {testStatus === "success" && (
                <div className="flex items-center text-success text-sm">
                  <CheckCircle2 className="h-4 w-4 mr-1" /> Ready to ingest
                </div>
              )}
              {testStatus === "error" && (
                <div className="flex items-center text-error text-sm">
                  <AlertCircle className="h-4 w-4 mr-1" /> Validation Failed
                </div>
              )}
                ...
                <CardHeader className="flex flex-row items-start justify-between pb-2">
                  <div className="space-y-1">
                    ...
                    <CardDescription className="pt-1">
                      {adapter.endpoint || "Local File Upload"}
                    </CardDescription>
                  </div>
                  <Badge
                    variant="success"
                  ...
                  <div className="flex items-center justify-between text-sm mt-4 pt-4 border-t border-card-border">
                    <div className="text-muted-foreground">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        <Badge
                          variant="outline"
                          className="text-[10px] bg-muted/50 text-white uppercase"
                        >
                          {adapter.mapping_mode} Mapping
                        </Badge>
                        <Badge
                          variant="outline"
                          className="text-[10px] bg-muted/50 text-white uppercase"
                        >
                          {adapter.connection_type}
                        </Badge>
                      </div>
                    </div>
