...
class GeminiLegacyParser:
    def __init__(self):
        # Gracefully look for GEMINI_API_KEY
        # Pull from settings.GEMINI_API_KEY if possible, fallback to os environment
    ...
    async def parse_legacy_file(self, file_content: str) -> LLMTransformationResult:
        ...
        try:
            # Call Gemini with strict structured output schema enforcement
            response = self.client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    ...
                    temperature=0.1 # Low temperature for accurate, deterministic extractions
                ),
            )
            
            # The response.text is guaranteed to match our LLMTransformationResult schema
